...
//        ()
        ...
        let rnd20 = Array.init N (fun i -> rnd.Next(2))
        let rnd100 = Array.init N (fun i -> rnd.Next(3))
        ...
        let xbeta = (2.2 + (new Vector(rnd20 |> Array.map float)) / 2.0 + (new Vector(rnd100 |> Array.map float)) / 3.0) + 3.3 * X
        ...
        let YVar = new Covariate("Y", new CovariateStorage(Y))
        let glm = Glm.fitModel YVar.AsExpr [!!AVar] true Gamma Ln 10000 50 1e-10
        ()
